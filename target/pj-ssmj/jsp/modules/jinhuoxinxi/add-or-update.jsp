    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑进货信息</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑进货信息</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">进货信息信息</h3>
                        <form id="addOrUpdateForm">
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-6">
                                    <label>进货数量</label>
                                    <input id="replenish" name="replenish" class="form-control"
                                           onchange="replenishChickValue(this)"  placeholder="进货数量">
                                </div>

                                   <div class="form-group col-md-6">
                                       <label>供应商</label>
                                       <select id="gyTypesSelect" name="gyTypes" class="form-control">
                                       </select>
                                   </div>
                                    <div class="form-group col-md-6">
                                        <label>经手员工</label>
                                        <select id="ygTypesSelect" name="ygTypes" class="form-control">
                                        </select>
                                    </div>
...
<script>
    ...
    //初始化下拉框
    function initializationGytypesSelect(){
        var gyTypesSelect = document.getElementById('gyTypesSelect');
            gyTypesSelect.add(new Option("==请选择==",""));
            for (var i = 0; i < gyTypesOptions.length; i++) {
                gyTypesSelect.add(new Option(gyTypesOptions[i].gysname,gyTypesOptions[i].id));
            }
    }
    function initializationYgtypesSelect(){
        var ygTypesSelect = document.getElementById('ygTypesSelect');
            ygTypesSelect.add(new Option("==请选择==",""));
            for (var i = 0; i < ygTypesOptions.length; i++) {
                ygTypesSelect.add(new Option(ygTypesOptions[i].name,ygTypesOptions[i].id));
            }
    }
    // 下拉框选项回显
    function setSelectOption() {
        for (var i = 0; i < gyTypesOptions.length; i++) {
            if(gyTypesOptions[i].id == ruleForm.gyTypes){//下拉框value对比,如果一致就赋值汉字
                document.getElementById("gyTypesSelect").options[i].selected = true;
            }
        }
        for (var i = 0; i < ygTypesOptions.length; i++) {
            if(ygTypesOptions[i].id == ruleForm.ygTypes){//下拉框value对比,如果一致就赋值汉字
                document.getElementById("ygTypesSelect").options[i].selected = true;
            }
        }
    }
    ...
    //搜素输入检查
            ...
            function replenishChickValue(e){
                var this_val = e.value || 0;
                var reg=/^[0-9]*$/;
                if(!reg.test(this_val)){
                    e.value = "";
    ...
    function dataBind() {
        ...
        $("#numbered").val(ruleForm.numbered);
        $("#name").val(ruleForm.name);
        $("#replenish").val(ruleForm.replenish);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
        initializationGytypesSelect();
        initializationYgtypesSelect();
        getDetails();
        //初始化上传按钮
        upload();
        //单列求和
